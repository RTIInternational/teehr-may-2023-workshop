...
def build_lt_hydrograph_from_query_selected_point(
    index: List[int],
    points_dmap: hv.DynamicMap,
    scenario: dict,
    value_time_start: Union[pd.Timestamp, None] = None,
    ...
    print("Biginning of build_lt_hydrograph_from_query_selected_point")
    ...
    if len(index) > 0 and len(points_dmap.dimensions('value')) > 0:  
        ...
        if 'value' in df.columns:  #single timeseries
            ...
            ts_layout_hv = (flow_prim * flow_sec * text_label).opts(show_title=False)
...
def build_lt_hydrograph_multi_from_query_selected_point(
    index: List[int],
    points_dmap: hv.DynamicMap,
    scenarios: List[dict],
    value_time_start: Union[pd.Timestamp, None] = None,
    ...
    # print("Biginning of build_lt_hydrograph_multi_from_query_selected_point")
...
#             )      
#             df = convert_query_to_viz_units(df, units, 'streamflow')
#             df_dict[scenario[i]["scenario_name"]] = df.copy(deep=true)
#             # print(df.head())
        # df_dict = {}
        df1 = run_teehr_query(
            query_type="timeseries",
        ...
        df1 = convert_query_to_viz_units(df1, units, 'streamflow') 
        df2 = run_teehr_query(
            query_type="timeseries",
        ...
        time_start = min(df1['value_time'].min(),df2['value_time'].min())
        time_end = max(df1['value_time'].max(),df2['value_time'].max())
        t = time_start + (time_end - time_start)*0.01
        text_x = t.replace(second=0, microsecond=0, minute=0).strftime('%Y-%m-%d-%H')
        ...
        if 'value' in df1.columns:  #single timeseries
            df1['normalized'] = df1['value']/area*conversion

            data_max = df1['value'].max()
            data_max_norm = df1['normalized'].max()
            ymax_flow = max(data_max*1.1,1)
            ymax_norm = max(data_max_norm*1.1,1)
            text_y = ymax_flow*0.9   
            text_label = hv.Text(text_x, text_y, title).opts(text_align='left', text_font_size='10pt', 
            ...
            flow = hv.Curve(df1, kdims = [('value_time', 'Date')], vdims = [('value', 'Flow ' + unit_label)])
            norm = hv.Curve(df1, kdims = [('value_time', 'Date')], vdims = [('normalized', 'Normalized Flow ' + unit_norm_label)])
            ...
            df1['primary_normalized'] = df1['primary_value']/area*conversion
            df1['secondary_normalized'] = df1['secondary_value']/area*conversion
            df2['primary_normalized'] = df2['primary_value']/area*conversion
            df2['secondary_normalized'] = df2['secondary_value']/area*conversion

            prim_max = df1['primary_value'].max()
            sec_max = max(df1['secondary_value'].max(),df2['secondary_value'].max())
            ...
            flow_prim = hv.Curve(df1, kdims = [('value_time', 'Date')], 
                                 vdims = [('primary_value', 'Flow ' + unit_label)], label='primary')
            # norm_prim = hv.Curve(df, kdims = [('value_time', 'Date')], 
            #                      vdims = [('primary_normalized', 'Normalized Flow ' + unit_norm_label)])
            
            flow_sec1 = hv.Curve(df1, kdims = [('value_time', 'Date')], 
                                vdims = [('secondary_value', 'Flow ' + unit_label)], label=scenarios[0]['scenario_name'])
            # norm_sec = hv.Curve(df, kdims = [('value_time', 'Date')], 
            #                     vdims = [('secondary_normalized', 'Normalized Flow ' + unit_norm_label)])

            flow_sec2 = hv.Curve(df2, kdims = [('value_time', 'Date')], 
                                vdims = [('secondary_value', 'Flow ' + unit_label)], label=scenarios[1]['scenario_name'])
            # norm_sec = hv.Curve(df, kdims = [('value_time', 'Date')], 
            #                     vdims = [('secondary_normalized', 'Normalized Flow ' + unit_norm_label)])

            flow_prim.opts(**opts, color='dodgerblue', ylim=(0, ymax_flow), backend='bokeh')
            # norm_prim.opts(**opts, color='blue', alpha=0, ylim=(0, ymax_norm), hooks=[plot_secondary_curve_curve])  
            
            flow_sec1.opts(**opts, color='sandybrown', ylim=(0, ymax_flow), backend='bokeh')
            # norm_sec1.opts(**opts, color='orange', alpha=0, ylim=(0, ymax_norm), hooks=[plot_secondary_curve_curve]) 
            
            flow_sec2.opts(**opts, color='red', ylim=(0, ymax_flow), backend='bokeh')
            # norm_sec2.opts(**opts, color='orange', alpha=0, ylim=(0, ymax_norm), hooks=[plot_secondary_curve_curve]) 

            ts_layout_hv = (flow_prim * flow_sec1 * flow_sec2 * text_label).opts(show_title=True)
            # ts_layout_hv = (flow_prim * flow_sec1 * text_label).opts(show_title=False)
        ...
        # print("RRR")
        df1 = pd.DataFrame([[0,0],[1,0]], columns = ['Date','value'])
        label = "Nothing Selected"
        curve = hv.Curve(df1, "Date", "value").opts(**opts, backend='bokeh')
        text = hv.Text(0.01, 0.9, "No Selection").opts(text_align='left', text_font_size='10pt', 
                                                       text_color='#57504d', text_font_style='bold')
        ts_layout_hv = (curve * text).opts(show_title=False)
...
def build_rose_diagram_from_query_selected_point(
    index: List[int],
    points_dmap: hv.DynamicMap,
    scenarios: List[dict],
    value_time_start: Union[pd.Timestamp, None] = None,
    ...
    if len(index) > 0 and len(points_dmap.dimensions('value')) > 0:  
        ...
        for i in range(5):
            r_circle = theta_circle * 0 + (i+1) * 0.2
            if i == 0:
                scatter0 = hv.Curve((theta_circle, r_circle), 'theta', 'r').redim(r=dict(range=(0,1))).opts(
                    alpha=0.5, color='gray', linewidth = 0.5, projection='polar', xticks=xticks, show_grid=True)
            else:
                scatter_temp = hv.Curve((theta_circle, r_circle), 'theta', 'r').redim(r=dict(range=(0,1))).opts(
                    alpha=0.5, color='gray', linewidth = 0.5, projection='polar', xticks=xticks, show_grid=True)
